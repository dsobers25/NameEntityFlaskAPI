import unittest # test runner library
...
class TestNerClient(unittest.TestCase):
    ...
    def test_get_ents_returns_dictionary_given_empty_string_causes_empty_spacy_doc_ents(self):
        model = NerModelTestDouble('eng') # something like a mock for the spacy library
    ...
    def test_get_ents_given_spacy_LOC_is_returned_serializes_to_Group(self):
        model = NerModelTestDouble('eng')
        doc_ents = [{'text': 'Kemet', 'label_':'LOC'}]
        model.returns_doc_ents(doc_ents)
        ner = NamedEntityClient(model)
        ents = ner.get_ents('...')
        expected_result = {'ents': [{'ent': 'Kemet', 'label': 'Location'}], 'html': ""}
        self.assertListEqual(ents['ents'], expected_result['ents'])
    ...
    def test_get_ents_given_multiple_ents_serializes_all(self):
        ...
        doc_ents = [{'text': 'New York', 'label_':'GPE'},
                    {'text': 'Deron Sobers', 'label_':'PERSON'}]
        model.returns_doc_ents(doc_ents)
        ner = NamedEntityClient(model)
        ents = ner.get_ents('...')
        expected_result = {'ents': 
            [{'ent': 'New York', 'label': 'Location'},
             {'ent': 'Deron Sobers', 'label': 'Person'}], 'html': ""}
        self.assertListEqual(ents['ents'], expected_result['ents'])